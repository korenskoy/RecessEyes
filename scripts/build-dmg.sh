...
# Читаем текущую версию (DMG-скрипт сам её не инкрементирует — этим
# занимается build_release.sh).
MARKETING_VERSION=$(grep "^MARKETING_VERSION" "$VERSION_FILE" | awk -F'=' '{print $2}' | tr -d ' ')
BUILD_NUMBER=$(grep "^CURRENT_PROJECT_VERSION" "$VERSION_FILE" | awk -F'=' '{print $2}' | tr -d ' ')

echo "▶ Packaging RecessEyes ${MARKETING_VERSION} (build ${BUILD_NUMBER})"

# Сборка Release, если артефакта ещё нет
